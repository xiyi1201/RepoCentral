...
* GENERATE SUB GROUP VARIABLES
*quietly: mean `subgroup'
*putexcel c`k'=(e(varlist))
...
gen age_6_10=1 if 6<=ridageyr<=10
gen age_11_14=1 if 11<=ridageyr<=14
gen age_15_19=1 if 15<=ridageyr<=19
gen age_20_34=1 if 20<=ridageyr<=34
gen age_35_49=1 if 35<=ridageyr<=49
gen age_50_69=1 if 50<=ridageyr<=69
gen age_70=1 if ridageyr>=70
gen female_19=1 if ridageyr<=19 & riagendr==2
gen male_19=1 if ridageyr<=19 & riagendr==1
gen female_20_49=1 if 20<=ridageyr<=49 & riagendr==2
gen male_20_49=1 if 20<=ridageyr<=49 & riagendr==1
gen female_50=1 if ridageyr>=50 & riagendr==2
gen male_50=1 if ridageyr>=50 & riagendr==1
gen NHW=1 if ridreth3==3
gen NHB=1 if ridreth3==4
gen Mexican_American=1 if ridreth3==1
*gen Hispanic=1 if ridreth3==2 /*2007-8 onwards*/
*gen NHA=1 if ridreth3==6 /*2011-12 onwards*/
...
*Is it self-defined income ratio threshold? 
gen lower_income=1 if indfmpir<=2.5
gen medium_income=1 if 2.5<indfmpir<5
gen higher_income=1 if indfmpir>=5
...
local k=1
* EXCEL LABELS: ADD PARAMETER, %_change, pairwise_test_last_2_cycles column
